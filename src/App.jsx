...
function Testimonials() {
  const quotes = [
    ['â€œWe stopped losing mornings to courier portals. The team can see what matters and act on it.â€', 'Rhea Mehta', 'Operations lead, Rooted Earth'],
    ['â€œThe rate view makes a complicated choice feel very human. We can move faster without guessing.â€', 'Arjun Shah', 'Founder, Northstar Goods'],
    ['â€œOur customers feel the difference because every update arrives with the right context.â€', 'Maya Iyer', 'CX manager, Common Thread'],
  ]
  return <section className="testimonials home-stack-card"><div className="shell"><div className="section-head compact"><div><Eyebrow icon={CircleCheck}>PEOPLE IN MOTION</Eyebrow><h2>Good journeys<br /><em>sound like this.</em></h2></div></div><div className="testimonial-grid">{quotes.map(([quote, name, role], index) => <article className="testimonial reveal-on-view" key={name}><div className="testimonial-stars">âœ¦ âœ¦ âœ¦ âœ¦ âœ¦</div><blockquote>{quote}</blockquote><div className="testimonial-person"><span>{name.slice(0, 1)}</span><div><strong>{name}</strong><small>{role}</small></div></div><i className="testimonial-line" style={{ '--delay': `${index * 0.8}s` }} /></article>)}</div></div></section>
}
