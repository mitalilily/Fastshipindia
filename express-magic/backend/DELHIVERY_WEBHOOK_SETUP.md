...
Use these URLs when asking Delhivery to enable Tracking Push API / Scan Push for FastShip.

## Required Scan Push URL

```text
https://api.fastship.in/api/webhook/delhivery/scan
```
...
```text
https://api.fastship.in/api/webhook/delhivery/document
```
...
```text
API_URL=https://api.fastship.in
```

Optional overrides are available for unusual hosting cases:

```text
DELHIVERY_SCAN_PUSH_WEBHOOK_URL=https://api.fastship.in/api/webhook/delhivery/scan
DELHIVERY_DOCUMENT_PUSH_WEBHOOK_URL=https://api.fastship.in/api/webhook/delhivery/document
DELHIVERY_LEGACY_WEBHOOK_URL=https://api.fastship.in/api/webhook/delhivery/order
```
