...
const DELHIVERY_B2B_DEV_API_HOST = 'ltl-clients-api-dev.delhivery.com'
...
export const normalizeDelhiveryB2BApiBase = (value: unknown) => {
  const normalized = clean(value).replace(/\/+$/, '')
  if (!normalized) return DEFAULT_DELHIVERY_B2B_API_BASE

  try {
    const parsed = new URL(normalized)
    if (parsed.hostname.toLowerCase() === DELHIVERY_B2B_DEV_API_HOST) {
      return DEFAULT_DELHIVERY_B2B_API_BASE
    }
...
export const getDelhiveryB2BCredentials = async (): Promise<DelhiveryB2BCredentials> => {
  ...
  return {
    apiBase: normalizeDelhiveryB2BApiBase(process.env.DELHIVERY_B2B_API_BASE || stored?.apiBase),
