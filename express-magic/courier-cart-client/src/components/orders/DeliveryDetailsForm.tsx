...
const DeliveryDetailsForm = ({ type = 'b2c' }: { type?: FormType }) => {
  ...
  const canSaveAddress = Boolean(
    currentAddress.address &&
      ...
      (type === 'b2b'
        ? currentAddress.companyName || currentAddress.buyerName
  ...
  const applySavedAddress = (savedAddress: SavedDeliveryAddress | null) => {
    ...
    if (type === 'b2b') {
      setValue(
        'companyName' as DeliveryFieldName,
        savedAddress.companyName || savedAddress.buyerName || savedAddress.label || '',
        {
          shouldDirty: true,
          shouldTouch: true,
          shouldValidate: true,
  ...
  return (
    ...
    >
      <Grid size={12}>
        ...
        {saveMessage ? (
          <Typography
            sx={{ mt: 0.7, color: canSaveAddress ? 'success.main' : 'text.secondary', fontSize: 12 }}
          >
            {saveMessage}
          </Typography>
        ) : null}
