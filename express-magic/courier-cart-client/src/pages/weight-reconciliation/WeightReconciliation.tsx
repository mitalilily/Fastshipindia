...
export default function WeightReconciliation() {
            ...
            bulkAccept.mutate(
                {
                    ...
                    notes: 'Bulk accepted from dashboard'
                },
                {
                    onSuccess: () => {
    ...
    const columns: Column<WeightDiscrepancy>[] = [
        ...
        { id: 'weight_difference', label: 'Difference (kg)', render: (value: unknown) => (
            <Typography component='span' sx={{ color: Number(value) > 0 ? '#E74C3C' : '#27AE60', fontWeight: 600, }}>
                {Number(value) > 0 ? '+' : ''} {(Number(value) / 1000).toFixed(3)}
            </Typography>),
        },
        { id: 'additional_charge', label: 'Extra Charge', render: (value: unknown) => `₹${Number(value || 0).toFixed(2)}`, },
        { id: 'status', label: 'Status', render: (value: unknown, row: WeightDiscrepancy) => {
            ...
            return <StatusChip label={displayLabel} status={statusColorMap[String(value)] || 'success'} />;
        },},
    ]
                    ...
                    <Box sx={{ ...panelSx, borderColor: isDark ? alpha(theme.palette.primary.main, 0.36) : '#333369', boxShadow: isDark ? cardShadow : '0 2px 8px rgba(59, 74, 116, 0.15)', p: 2 }}>
                        <Stack direction='row' alignItems='center' justifyContent='space-between'>
                            ...
                            <Stack direction='row' gap={2}>
                                ...
                                <Button variant='outlined' onClick={handleBulkReject} disabled={bulkReject.isPending} sx={{ borderColor: '#E74C3C', color: '#E74C3C', textTransform: 'none', '&:hover': { borderColor: '#C0392B', backgroundColor: 'rgba(231, 76, 60, 0.08)', }, }}>
                                    Reject Selected
                                </Button>
                            </Stack>
                        </Stack>
