...
function Contact() { const [sent, setSent] = useState(false); return <section className="contact-page"><div className="shell contact-grid"><div><Eyebrow icon={Headphones}>LET'S TALK</Eyebrow><h1>Bring us the route you are thinking about.</h1><p>Share your volume, your destinations or the operational snag you want to untangle. We will help map a clearer way forward.</p><div className="contact-options"><div className="contact-option"><span className="contact-option-icon"><Phone /></span><small>Call to ask any question</small><a href="tel:+918285681158">+91 82856 81158</a><a href="tel:+917360913725">+91 73609 13725</a></div><div className="contact-option"><span className="contact-option-icon"><Mail /></span><small>Email to get free quote</small><a href="mailto:hello@fastshipindia.com">hello@fastshipindia.com</a></div><div className="contact-option"><span className="contact-option-icon"><MapPin /></span><small>Visit our office</small><strong>Prince Kataria Complex, Gali No. 81A, Laxman Vihar Phase 2, Gurgaon, Haryana 122001</strong></div></div></div><form className="contact-form" onSubmit={e => { e.preventDefault(); setSent(true); e.currentTarget.reset() }}><Eyebrow icon={Sparkles}>START A CONVERSATION</Eyebrow><label>Your name<input required name="name" placeholder="Full name" /></label><label>Work email<input required type="email" name="email" placeholder="you@company.com" /></label><label>What are you moving?<textarea required rows="5" name="message" placeholder="Tell us a little about the journey" /></label><button className="button button-coral full" type="submit">Send enquiry <ArrowRight size={17} /></button>{sent && <p className="success"><CircleCheck size={16} /> Thanks, we will be in touch shortly.</p>}</form></div></section> }
...
function Footer() { return <footer><div className="shell footer-grid"><div><Logo light /><p>Shipping infrastructure with a little more clarity, care and momentum.</p><div className="socials"><a href="#" aria-label="LinkedIn"><Linkedin /></a><a href="#" aria-label="Instagram"><Instagram /></a></div></div><div><h4>Explore</h4><Link to="/integrations">Platform</Link><Link to="/integrations/courier-partners">Courier network</Link><Link to="/blogs">Journal</Link></div><div><h4>Tools</h4><Link to="/rate-calculator">Rate calculator</Link><Link to="/weight-calculator">Weight calculator</Link><Link to="/tracking">Track a parcel</Link></div><div><h4>Say hello</h4><a href="mailto:hello@fastshipindia.com">hello@fastshipindia.com</a><a href="tel:+918285681158">+91 82856 81158</a><a href="tel:+917360913725">+91 73609 13725</a><Link to="/contact">Contact page <ArrowRight size={14} /></Link></div></div><div className="shell footer-bottom"><span>{'\u00a9'} 2026 Fastship India</span><span>Privacy {'\u00b7'} Terms {'\u00b7'} Security</span></div></footer> }

function PageRouter() {
  return <Routes>
    <RouterRoute path="/" element={<Home />} />
    <RouterRoute path="/integrations" element={<StandardPage type="integrations" />} />
    <RouterRoute path="/integrations/sales-channels" element={<StandardPage type="salesChannels" />} />
    <RouterRoute path="/integrations/courier-partners" element={<StandardPage type="courierPartners" />} />
    <RouterRoute path="/blogs" element={<StandardPage type="blogs" />} />
...
export default function App() { return <><RouteEffects /><Header /><main><PageRouter /></main><Footer /></> }
